    ...
    while (child || index < selectionSet.length) {
      ...
      if (child) {
        ...
        if (!shouldInclude(select, ctx.variables)) ; else if (select.kind !== graphql_web.Kind.FIELD) {
          ...
          if (fragment) {
            var isMatching = !fragment.typeCondition || (ctx.store.schema ? isInterfaceOfType(ctx.store.schema, fragment, typename) : currentOperation === 'read' && isFragmentMatching(fragment.typeCondition.name.value, typename) || isFragmentHeuristicallyMatching(fragment, typename, entityKey, ctx.variables, ctx.store.logger));
            if (isMatching || currentOperation === 'write' && !ctx.store.schema) {
              ...
              if (isMatching && fragment.typeCondition && typename !== fragment.typeCondition.name.value) {

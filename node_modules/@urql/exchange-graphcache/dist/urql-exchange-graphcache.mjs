...
import { share as s, map as l, filter as v, merge as d, mergeMap as u, empty as p, fromArray as c, makeSubject as f, onPush as y } from "wonka";
  ...
  return function next() {
    ...
    while (s || l < i.length) {
      ...
      if (s) {
        ...
        var d = i[l++];
        if (!shouldInclude(d, o.variables)) {} else if (d.kind !== n.FIELD) {
          var u = d.kind !== n.INLINE_FRAGMENT ? o.fragments[getName(d)] : d;
          if (u) {
            var p = !u.typeCondition || (o.store.schema ? isInterfaceOfType(o.store.schema, u, e) : "read" === D && isFragmentMatching(u.typeCondition.name.value, e) || isFragmentHeuristicallyMatching(u, e, r, o.variables, o.store.logger));
            if (p || "write" === D && !o.store.schema) {
              if ("production" !== process.env.NODE_ENV) {
                pushDebugNode(e, u);
              }
              var c = isOptional(d);
              if (p && u.typeCondition && e !== u.typeCondition.name.value) {
                writeConcreteType(u.typeCondition.name.value, e);
              }
...
var readSelection = (e, r, t, a, i) => {
  ...
  while (void 0 !== (c = v())) {
    ...
    if ("__typename" === h) {
      ...
      b = N(x, m || {}, n, e);
      if (c.selectionSet) {
        b = resolveResolverResult(e, l, h, E, getSelectionSet(c), void 0 !== y[g] ? y[g] : a[g], b, ownsData(a));
      }
      ...
      b = resolveResolverResult(e, l, h, E, getSelectionSet(c), void 0 !== y[g] ? y[g] : a[g], w, ownsData(a));
    } else {
      var I = readLink(s, k);
      if (void 0 !== I) {
        b = resolveLink(e, I, l, h, getSelectionSet(c), void 0 !== y[g] ? y[g] : a[g], ownsData(a));
      } else if ("object" == typeof O && null !== O) {
        b = O;
      }
    }
    if (!S && void 0 === b && (_.optional || T && !_.required || getFieldError(e) || n.schema && isFieldNullable(n.schema, l, h, e.store.logger))) {
      e.partial = !0;
      b = null;
...
var writeSelection = (e, r, t, a) => {
  ...
  while (v = l()) {
    ...
    if (void 0 === f) {
      if ("production" !== process.env.NODE_ENV) {
        if (!r || !hasField(r, p) || e.optimistic && !readRecord(r, "__typename")) {
          "production" !== process.env.NODE_ENV && warn("Invalid undefined: The field at `" + p + "` is `undefined`, but the GraphQL query expects a " + (void 0 === v.selectionSet ? "scalar (number, boolean, etc)" : "selection set") + " for this field.", 13, e.store.logger);
        }
      }
      continue;
    }
    if (v.selectionSet) {
      if (r && "query" === i) {
        var h = joinKeys(r, p);
        var m = writeField(e, getSelectionSet(v), ensureData(f), h, e.optimistic ? readLink(r || o, p) : void 0);
        writeLink(r || o, p, m);
      } else {
...
var writeField = (e, r, t, a, i) => {
  ...
  var u = t.__typename;
  if ("production" !== process.env.NODE_ENV) {
    if (a && !e.store.keys[t.__typename] && null === d && "string" == typeof u && !M.test(u)) {
...
var offlineExchange = e => r => {
  ...
  if (n && n.onOnline && n.readMetadata && n.writeMetadata) {
    var {forward: l, client: u, dispatchDebug: p} = r;
